...
void draw()
{
  ...
  if (finishTime!=0)
  {
    ...
    text("Total time taken: " + ((finishTime - startTime)/1000f),400,220); //output
  ...
  /* BAKEOFF 3 Changes - START */
  drawKeyboard(); // Draw the on-screen keyboard
  /* BAKEOFF 3 Changes - END */
  ...
  if (startTime!=0)
  {
    ...
    //Don't need if we use alphabet letters with space & delete
...
void drawKeyboard() {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      float x = keyboardX + j * keyWidth;
      float y = keyboardY + i * keyHeight;
      fill(200);
      rect(x, y, keyWidth, keyHeight);
      fill(0);
      textAlign(CENTER, CENTER);
      
      if (cell == 27) { // Assign one of the last keys as the space key
        fill(0, 255, 0); //green button
        text("sp", x + keyWidth / 2, y + keyHeight / 2);
      } else if (letter <= 'z') {
        text(letter, x + keyWidth / 2, y + keyHeight / 2);
        letter++;
      } else {
        
        if(cell == 28){
          fill(255,0,0);
          text("bk", x + keyWidth / 2, y + keyHeight / 2);
        }
        if(cell == 30){
          fill(255,0,0);
          text("en", x + keyWidth / 2, y + keyHeight / 2);
        }
      }
      cell++;
    }//cols
...
// Check if the click is within the keyboard area
  if (mouseX >= keyboardX && mouseX < keyboardX + sizeOfInputArea &&
    ...
    if (index < 26) { // Alphabet keys
      currentTyped += char('a' + index);
    } else if (index == 26) { // Space key
      currentTyped += ' ';
    } else if (index == 27) { // Delete key
      if (currentTyped.length() > 0) {
        currentTyped = currentTyped.substring(0, currentTyped.length() - 1);
      }
    }else if (index == 29) { //check if click is in next button
