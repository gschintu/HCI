...
void draw() {
  ...
  noStroke();
   ...
   if(!gameStarted) {
    ...
    text("**- Click on left, right, up, and down buttons to move blue square in the proper direction  -**", width / 2, instructionsHeight + 160);
    text("**- Click on Counterclockwise(CCW) and Clockwise(CW) to rotate blue square -**", width / 2, instructionsHeight + 200);
    fill(255, 0, 0, 200); // set fill color to translucent red
    text("**- Make sure you press Next Trial button only when flashing, this way you know it is a match -**", width / 2, instructionsHeight + 240);
    fill(255);
    
    // Start Challenge Button
    fill(0, 0, 255); // blue color for the button background
    rect((width / 2), instructionsHeight+280, btnWidth, btnHeight,28);
    fill(255); // black color for the text
    text("Start Challenge", width / 2, instructionsHeight + 288);
    btnX = (width / 2) - (btnWidth / 2);
    btnY = (instructionsHeight + 260); // it's not quite aligned with how the rect is built.
